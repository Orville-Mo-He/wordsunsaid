...
function setup() {
    ...
    for (let i = 0; i < blocks.length; i++) {
        unsaid[i] = new Unsaid(random(width), random(height), random(0.0005, 0.001), random(0.0005, 0.001), 0 - (200 * i), false, 0, 0);
    }
...
function draw() {
    ...
    //used blocks2 because I wanted specific spacing (achieved by putting extra spaces) for the actual array.
    for (let i = 0; i < blocks2.length; i++){
        alltheheights[i]= textHeight(blocks2[i], unsaid.lineLength, 17);
    };
    ...
    // test.show();
    // test.upDown(height / 24);
    // for (let i = 0; i < 5; i++) {
    //     tests[i].show()
    //     tests[i].upDown(random())
    // }
...
class Unsaid {
    ...
    move() {
        ...
        this.xOut = map(noise(this.xseed), 0, 1, 0, width - (this.lineLength / 2));
    ...
    words(_a, _i) { //_a = what to type. the many many texts so blurred.
        ...
        textSize(14)
        textAlign(LEFT, CENTER);
        this.pyCenter = alltheheights[_i]; 
